...
window.joyviz = {};
window.$t = window.joyviz;

joyviz.copyto = function(obj, res) {
...
joyviz.copy = function(obj) {
    if (!obj) return obj;
    return joyviz.copyto(obj, new obj.constructor());
}

joyviz.element = function(name, props, place, content) {
...
joyviz.inner = function(obj, sel) {
    sel.appendChild(obj.nodeName != undefined ? obj : document.createTextNode(obj));
    return sel;
}

joyviz.id = function(id) {
    return document.getElementById(id);
}

joyviz.set = function(sel, props) {
    for (var i in props) sel.setAttribute(i, props[i]);
    return sel;
}

joyviz.clas = function(sel, oldclass, newclass) {
...
joyviz.empty = function(sel) {
    if (sel.childNodes)
        while (sel.childNodes.length)
            sel.removeChild(sel.firstChild);
}

joyviz.remove = function(sel) {
    if (sel) {
...
joyviz.bind = function(sel, eventname, func, bubble) {
    if (!sel) return;
...
joyviz.unbind = function(sel, eventname, func, bubble) {
    if (eventname.constructor === Array) {
        for (var i in eventname)
...
joyviz.one = function(sel, eventname, func, bubble) {
    var one_func = function(e) {
        func.call(this, e);
        joyviz.unbind(sel, eventname, one_func, bubble);
    };
    joyviz.bind(sel, eventname, one_func, bubble);
}

joyviz.raise_event = function(sel, eventname, bubble, cancelable) {
...
joyviz.raise = function(sel, eventname, params, bubble, cancelable) {
...
if (!document.getElementsByClassName) {
    joyviz.get_elements_by_class = function(classes, node) {
...
joyviz.rpc = function(params, callback, noparse) {
        ...
        promise: function() {
            return {
                then: function(callback) {
                    var deferred = joyviz.deferred(), promise = deferred.promise();
...
joyviz.when = function(promises) {
    var deferred = joyviz.deferred();
